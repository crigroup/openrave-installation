...
if [ ${UBUNTU_VER} = '14.04' ]; then
  sudo apt-get install -qq -y --no-install-recommends qt4-dev-tools zlib-bin
elif [ ${UBUNTU_VER} = '16.04' ] || [ ${UBUNTU_VER} = '18.04' ]; then
  sudo apt-get install -y --no-install-recommends qt5-default minizip
fi
# Libraries
sudo apt-get install -qq -y --no-install-recommends ann-tools libann-dev        \
...
if [ ${UBUNTU_VER} = '14.04' ]; then
  sudo apt-get install -qq -y --no-install-recommends collada-dom-dev libccd    \
  libpcrecpp0 liblog4cxx10-dev libqt4-dev
elif [ ${UBUNTU_VER} = '16.04' ] || [ ${UBUNTU_VER} = '18.04' ]; then
  sudo apt-get install -qq -y --no-install-recommends libccd-dev                \
  libcollada-dom2.4-dp-dev liblog4cxx-dev libminizip-dev octomap-tools
fi

# Install boost
if [ ${UBUNTU_VER} = '14.04' ] || [ ${UBUNTU_VER} = '16.04' ]; then
    sudo apt-get install -qq -y --no-install-recommends libboost-all-dev        \
    libboost-python-dev
